<template>
  ...
  const activeSlide = ref(store.state.indexCarousel); // Initialize activeSlide with indexCarousel from the store
  const weatherForecasts = ref(store.state.weatherForecasts); // Initialize weatherForecasts with state.weatherForecasts
  ...
  // Watch for changes in activeSlide and update the store
  watch(activeSlide, (newValue) => {
    store.commit('setIndexCarousel', newValue);
  });
  
  // Watch for changes in store's indexCarousel and update activeSlide
  watch(() => store.state.indexCarousel, (newValue) => {
    activeSlide.value = newValue;
  });
  
  // Watch for changes in store's weatherForecasts and update weatherForecasts
  watch(() => store.state.weatherForecasts, (newValue) => {
    weatherForecasts.value = newValue;
  });
  ...
  const fetchWeatherForecasts = async (latitude, longitude) => {
    try {
      ...
      if (data.hourly && data.hourly.time && data.hourly.temperature_2m && data.hourly.relative_humidity_2m) {
        ...
        const forecasts = data.hourly.time.slice(startIndex, startIndex + 10).map((time, index) => {
        ...
        // Save weather forecasts to the store
        store.commit('setWeatherForecasts', forecasts);
      } else {
